...
parser.add_argument("--member", help="Member to fit", type=int, required=True)
parser.add_argument("--month", help="month to fit", type=int, required=True, default=6)
...
parser.add_argument(
    ...
    default="/scratch/hadsx/cpm/5km/daily/1day/"
)
args = parser.parse_args()
opdir = "%s/%s/%s" % (args.opdir, args.variable, "normalised")
if not os.path.isdir(opdir):
    os.makedirs(opdir, exist_ok=True)


# Go through data and estimate min and mean (for each day)
...
mean = tf.zeros([1, 244, 180, 1], dtype=tf.float32)
count = tf.zeros([1, 244, 180, 1], dtype=tf.float32)
for batch in trainingData:
    month = 6 # int(batch[1].numpy()[0][5:7])
    # if month == args.month:
...
variance = tf.zeros([244, 180, 1], dtype=tf.float32)
for batch in trainingData:
    month = 6 #int(batch[1].numpy()[0][5:7])
    if month == args.month:
...
fg_location = tf.zeros([1, 244, 180, 1], dtype=tf.float32)
