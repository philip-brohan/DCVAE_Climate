...
def getFileNames(variable, startyear=1850, endyear=2050):
    inFiles = sorted(
        os.listdir("/scratch/hadsx/cpm/5km/daily/1day/%s/raw_tensors" % (variable))
    )
    # inFiles = [
...
def getDataset(variable, startyear=1850, endyear=2050, blur=None, cache=False):
    ...
    fnFiles = [
        "/scratch/hadsx/cpm/5km/daily/1day/%s/raw_tensors/%s" % (variable, x)
        for x in inFiles
    ]
    ...
    if blur is not None:
        ts_data = ts_data.map(
            lambda x: x + tf.random.normal([244, 180, 1], stddev=blur),
