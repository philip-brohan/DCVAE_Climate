...
# For a specified day and variable - shape, location, and scale.
...
parser.add_argument(
    "--day", help="Day to plot", type=int, required=False, default=3
)
parser.add_argument(
    "--variable",
...
shape = iris.load_cube(
    "%s/DCVAE-Climate/normalization/CPM5/%s/shape_m%02d.nc"
    % (os.getenv("SCRATCH"), args.variable, args.day),
)
location = iris.load_cube(
    "%s/DCVAE-Climate/normalization/CPM5/%s/location_m%02d.nc"
    % (os.getenv("SCRATCH"), args.variable, args.day),
)
scale = iris.load_cube(
    "%s/DCVAE-Climate/normalization/CPM5/%s/scale_m%02d.nc"
    % (os.getenv("SCRATCH"), args.variable, args.day),
)
