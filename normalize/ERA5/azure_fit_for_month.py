...
# Run the normalization fits process on azure
...
# define the job
command_job = command(
    name="make_normalization_fit_%s" % datetime.now().strftime("%Y%m%d%H%M%S"),
    compute="cpu-cluster",
    ...
    + "python normalize/ERA5/fit_for_month.py --variable=%s --month=%d "
    % (args.variable, args.month),
)
